"""Native CPU compilation and execution for the Nano v1 Direct path.
...
class NativeKernel:
    ...
    def run_into(
        ...
        self._function(*(ctypes.byref(descriptor) for descriptor in descriptors))
        return output_value
...
class NativeCpuCompiler:
    """Compile and cache Nano v1 Direct contractions for the host CPU."""
    ...
    def lower(self, problem: ContractionProblem, stage: str) -> str:
        """Return real pipeline output for ``tc``, ``linalg``, or LLVM stages."""
        ...
        if stage not in ("llvm", "llvm-ir"):
            raise ValueError(
                "unknown native lowering stage '{}'; expected tc, linalg, llvm, or llvm-ir".format(
                    stage
                )
            )
        llvm_dialect = _run(
            (str(self.toolchain.optimizer), "-", *NATIVE_PIPELINE),
            input_text=source,
            timeout_seconds=self.timeout_seconds,
        )
        if stage == "llvm":
    ...
    def compile(self, problem: ContractionProblem) -> NativeKernel:
        if fcntl is None:
            raise BackendError(
                "native artifact locking requires a POSIX host (macOS or Linux)"
            )
        source = self._source(problem, c_interface=True)
        key, identity = self._cache_identity(problem, source)
        ...
        try:
            with lock_path.open("a+b") as lock_handle:
                ...
                try:
                    self._compile_entry(
                        problem, source, temporary, key, identity
                    )
                    if entry.exists():
